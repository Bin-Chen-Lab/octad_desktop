...
CodeFolder = '../Code/'
dir(CodeFolder)
outputFolder = 'Workflow-BRCA-LumA-AllGTEX/'
...
phenoDF_case <- phenoDF %>% 
  filter(cancer == 'Breast Invasive Carcinoma',
         sample.type %in% c('primary'),gender=='Female',subtype=='LumA')
dim (phenoDF_case)
head (phenoDF_case)
...
phenoDF_control = phenoDF %>% filter(biopsy.site == 'BREAST - MAMMARY TISSUE',
                                     gender=='Female')
control_id = phenoDF_control$sample.id
...
k = 1 # default 1
for(i in c(1000,5000,15000)){
n_topGenes = i # default 10000 for nondemo
...
write.csv(res,file=paste0(outputFolder,'res_geneinfo_','k',k,'_n',n_topGenes,'.csv'))  
}
...
#baseline
res_adj = read.csv('Workflow-BRCA-LumA-Adjacent_noNorm/res_geneinfo.csv',stringsAsFactors = F)

baseline = c(
res_adj %>% nrow(),
res_adj %>% filter(padj<0.001) %>% nrow,
res_adj %>% filter(padj<0.001,abs(log2FoldChange)>1) %>% nrow,
res_adj %>% filter(padj<0.001,abs(log2FoldChange)>1.5) %>% nrow,
res_adj %>% filter(padj<0.001,abs(log2FoldChange)>2) %>% nrow)


res = read.csv('Workflow-BRCA-LumA-AllGTEX/res_geneinfo_unNorm.csv',stringsAsFactors = F)

unNorm = c(
res %>% nrow(),
res %>% filter(padj<0.001) %>% nrow,
res %>% filter(padj<0.001,abs(log2FoldChange)>1) %>% nrow,
res %>% filter(padj<0.001,abs(log2FoldChange)>1.5) %>% nrow,
res %>% filter(padj<0.001,abs(log2FoldChange)>2) %>% nrow)
unNorm


joinedres = res %>% select(id,log2FoldChange,padj) %>% inner_join(res_adj,by='id')
nrow(joinedres)
joinedres %>% filter(padj.x<0.001,padj.y<0.001) %>% nrow()
joinedres %>% filter(padj.x<0.001,padj.y<0.001,abs(log2FoldChange.x)>1,abs(log2FoldChange.y)>1) %>% nrow()
joinedres %>% filter(padj.x<0.001,padj.y<0.001,abs(log2FoldChange.x)>1.5,abs(log2FoldChange.y)>1.5) %>% nrow()
joinedres %>% filter(padj.x<0.001,padj.y<0.001,abs(log2FoldChange.x)>2,abs(log2FoldChange.y)>2) %>% nrow()

cor.test(joinedres$log2FoldChange.x,joinedres$log2FoldChange.y)
tmp = joinedres %>% filter(padj.x<0.001,padj.y<0.001)
cor.test(tmp$log2FoldChange.x,tmp$log2FoldChange.y)
...
***
```{r}
res_adjEnR = geneEnrich(res_adj %>% filter(padj<0.001))
res_adjEnR1 = geneEnrich(res_adj %>% filter(padj<0.001,abs(log2FoldChange)>1))
res_adjEnR1.5 = geneEnrich(res_adj %>% filter(padj<0.001,abs(log2FoldChange)>1.5))
res_adjEnR2 = geneEnrich(res_adj %>% filter(padj<0.001,abs(log2FoldChange)>2))

res_EnR = geneEnrich(res %>% filter(padj<0.001))
res_EnR1 = geneEnrich(res %>% filter(padj<0.001,abs(log2FoldChange)>1))
res_EnR1.5 = geneEnrich(res %>% filter(padj<0.001,abs(log2FoldChange)>1.5))
res_EnR2 = geneEnrich(res %>% filter(padj<0.001,abs(log2FoldChange)>2))
```
...
#dz_sigUsed = read.csv('Workflow-HCC-A-2018-09-28/dz_sig_used.csv')
...
```{r Drug Enrichment}
outputFolder = 'Workflow-BRCA-LumA-AllGTEX/'
srges = read.csv('Workflow-BRCA-LumA-AllGTEX//sRGES.csv')
pipelineDataFolder = '../../Data/'
