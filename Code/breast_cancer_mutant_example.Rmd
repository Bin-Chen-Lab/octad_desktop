...
mut_only       <- (mutant.df.wide %>% filter(TP53 == 1))$Sample.id # change "MYC" to your mutation of interest
mut_onlydz     <- intersect(mut_only,allbrcaids)
phenoDF_case   <- filter(phenoDF,sample.id %in% as.character(mut_onlydz))
...
write(case_id, file = paste0(outputFolder,'case_ids.txt'))
```
...
(top_refs = reference_tissue_rank %>% arrange(desc(medianCor)))
normal_median_cor$ref <- factor(normal_median_cor$biopsy.site, levels = top_refs$biopsy.site)
normal_median_cor$control <- NA
normal_median_cor$control[1:50] <- 'control'
top10 = levels(normal_median_cor$ref)[1:10]
```
...
p <- ggplot(normal_median_cor %>% filter(ref %in% top10), aes(ref, cor))
...
########## To evaluate other n_topGenes settings ##########
# for(i in c(1000,5000,15000)){
# n_topGenesLoop = i # default 10000
# DE_method = 'edgeR' # Default 'edgeR'. Other choice is 'limma'
# # logging parameters
# write(paste0("n_topGenes = ",n_topGenesLoop), file = paste0(outputFolder,"parameters.txt"),append = TRUE)
# fileConn<-file(paste0(outputFolder,"parameters.txt"))
# close(fileConn)
...
#         k=k,
#         n_topGenes=n_topGenesLoop,
#         DE_method=DE_method
#         )
# res_geneinfo <- left_join(res, merged_gene_info, by=c('identifier'='ensembl'))
# write.csv(res,file=paste0(outputFolder,'res_geneinfo_',"n_topGenes_",n_topGenesLoop,'.csv'))
# }
