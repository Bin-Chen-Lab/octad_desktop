...
lincs_sig_info <- data.table::fread(paste0(pipelineDataFolder,"/LINCS_RGES/lincs_sig_info.csv"),stringsAsFactors = F)
...
####summarize Lincs RGES Scores####
summarizeLincsRGES = function(lincs_rges,weight_cell_line = F){
...
pred_merged = summarizeLincsRGES(results)
...
#for testing purposes only doesn't really make sense to merge random_cmap_scores
#this is b/c w/i the loop of each score the dz_gene used is randomized everytime
#randomResults <- data.frame(id = random_sig_ids, cmap_score = random_cmap_scores)
#randomResults <- merge(randomResults, lincs_sig_info, by = "id")
#randomResults <- results[order(randomResults$cmap_score),]
#randomSRGES = summarizeLincsRGES(randomResults)

#write.csv(randomResults, output_path)


pred_merged$p <- sapply(pred_merged$sRGES, function(score){
  sum(random_cmap_scores < score)/length(random_cmap_scores)
})

pred_merged$padj <- p.adjust(pred_merged$p, "fdr")
