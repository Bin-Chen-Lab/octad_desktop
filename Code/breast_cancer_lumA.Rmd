...
# source(paste0(CodeFolder,'drugs_core_functions.R'))
source(paste0(CodeFolder,'core_functions.R'))
targets = c('chembl_targets','mesh','meshes','sea_targets','ChemCluster') # include as many as you prefer. They take quite a long time each to run!!
...
#   write.csv(gsea_p, paste0(enrichFolder.n, "/enriched_sRGES_all", target_type, ".csv"))
...
#   pdf(paste0(enrichFolder.n, "/top_enriched_sRGES_all_", target_type, ".pdf"))
...
# clusternames <- as.character((gsea_summary[which(gsea_summary$padj<=0.05),])$target)
