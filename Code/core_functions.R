...
topLineEvalv2 <- function(topline = c('')){
...
visualize_dz_sigV2 <- function(case_id, control_id){
  library(gplots)
  library(pheatmap)
  ...
  df <- data.frame(row.names = c(case_id, control_id), type= c(rep("case", length(case_id)), rep("control", length(control_id))))
  colnames(df) <-c("type")
  mycol <- colorRampPalette(rev(brewer.pal(n = 6, name =
                                             "RdYlBu")))(100)
  set.seed(0)
  pheatmap(expr, 
           cluster_rows=T, 
           ...
           cluster_cols=F,
           annotation_col=df, 
           file= paste0(outputFolder, "enrichment_analysis/GeneEnrichment/signature.pdf"))
