...
* sRGES.csv
```{r}
outputFolder = paste0('demo_dev_output-',Sys.Date(),'/')
...
phenoDF = fread(paste0(pipelineDataFolder,'integrated.OCTAD.basic.sample.meta.csv'),stringsAsFactors = F)
...
```{r}
source(paste0(WebPortalCodeFolder,'DE_core_functions_v2.R'))
...
```{r, message=FALSE, warning=FALSE}
gene_info <- fread(paste0(pipelineDataFolder,'/gene_info_hs.csv'),stringsAsFactors = F)
