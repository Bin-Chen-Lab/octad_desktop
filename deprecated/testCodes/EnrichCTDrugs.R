...
library(“GSVA”)
library(“colorRamps”)
dz_drugs <- read.csv(“~/Downloads/4. Previous Tested Comparison SRGES NCAT.csv”,stringsAsFactors = F)
#dz_drugs <- queryClinicalTrials(‘Oligodendroglioma’)

#grabs only the drugs in the validation set
database <- “lincs”
sRGES = read.csv(“~/Downloads/sRGES_50.csv”)
sRGES$drug <- 0
sRGES$drug[tolower(sRGES$pert_iname) %in% tolower(dz_drugs$pert_iname)] = 1
...
pdf( “~/Downloads/clinical_drugs_validation.pdf”, sep=“”))
