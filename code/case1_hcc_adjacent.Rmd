...
outputFolder = paste0(base.folder,'/results/case1_hcc/')
...
***
For this demo we are only selecting HCC adjacent
```{r case.load3}
setwd(base.folder)
phenoDF_adjacent <- phenoDF %>% filter(cancer == 'Liver Hepatocellular Carcinoma', sample.type == 'adjacent', data.source == 'TCGA')
control_id = (phenoDF_adjacent)$sample.id
...
sRGES = read.csv('~/octad_desktop/results/case1_hcc/sRGES.csv',stringsAsFactors = F)
LIHCdrugs = read.csv('../octad_case/dataset/LIHC_rges_ic50_normalized.csv',stringsAsFactors = F)
LIHCdrugsFull = read.csv('../octad_case/dataset/LIHC_sRGES_published.csv',stringsAsFactors = F)
LIHCdrugs$pert_iname = toupper(LIHCdrugs$pert_iname)
LIHCdrugsFull$pert_iname = toupper(LIHCdrugsFull$pert_iname)
sRGES$pert_iname = toupper(sRGES$pert_iname)
sRGESval = inner_join(sRGES,LIHCdrugs,by='pert_iname',suffix=c('.octad','.natcomm'))
sRGESvalfull = inner_join(sRGES,LIHCdrugsFull,by='pert_iname',suffix=c('.octad','.natcommfull'))
plot(sRGESvalfull$sRGES.octad,sRGESvalfull$sRGES.natcommfull)
cor.test(sRGESvalfull$sRGES.octad,sRGESvalfull$sRGES.natcommfull)
